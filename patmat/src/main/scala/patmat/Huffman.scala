...
object Huffman {
   ...
   */
    def until(first: List[CodeTree]=>Boolean, second: List[CodeTree]=>List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (first(trees)) trees
      else until(first, second) (second(trees))
    }
