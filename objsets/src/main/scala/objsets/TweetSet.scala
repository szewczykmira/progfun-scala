...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted : Tweet = {
      lazy val lefty = left.mostRetweeted
      lazy val righty = right.mostRetweeted

      if( !left.empty && lefty.retweets > elem.retweets )
        if( !right.empty && righty.retweets > lefty.retweets )
          righty
        else
          lefty
      else if( !right.empty && righty.retweets > elem.retweets )
        righty
      else
        elem
    }
